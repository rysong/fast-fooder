      ...
      if (action == "change_color") {
        $("head").append(
          $('<link rel="stylesheet" type="text/css">').attr(
            "href",
            "assets/css/temporary-style.css"
          )
        );
        $("link").each(function() {
          if (
...
function createCustomizer(defaultColor) {
  ...
  $.getScript("assets/js/icheck.min.js", function(data, textStatus, jqxhr) {
    if ($("input[type=checkbox]").length > 0) {
      ...
      $(".customizer #checkbox-fixed-navigation").on("ifChecked", function(
        event
      ) {
        fixedNavigation(true);
      });
      $(".customizer #checkbox-fixed-navigation").on("ifUnchecked", function(
        event
      ) {
      ...
      $(".customizer #checkbox-nav-btn-only").on("ifUnchecked", function(
        event
      ) {
