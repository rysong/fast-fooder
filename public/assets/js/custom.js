...
$(document).ready(function($) {
  ...
  $("select").on("rendered.bs.select", function() {
    ...
    if (!viewport.is("xs")) {
      $(".search-form.vertical").css(
        "top",
        $(".hero-section").height() / 2 -
  ...
  if (!viewport.is("xs")) {
    $(".search-form.vertical").css(
      "top",
      $(".hero-section").height() / 2 - $(".search-form .wrapper").height() / 2
    );
    trackpadScroll("initialize");
  }
  ...
  if ($(".calendar").length) {
    ...
    for (var i = 1; i <= 12; i++) {
      $(".calendar-wrapper").append(
        '<div id="month_' + i + '" class="month"></div>'
      );
  ...
  $(".form-email .btn[type='submit']").on("click", function() {
    ...
    form.validate({
      submitHandler: function() {
        $.post("assets/external/email.php", form.serialize(), function(
          response
        ) {
...
function heroSectionHeight() {
  if ($(".hero-section").length > 0) {
    if (viewport.is("xs")) {
      ...
      $(".hero-section").height(
        $(".hero-section .map-wrapper").height() +
          $(".hero-section .search-form").height() +
          $(".hero-section .results").height() +
          40
      );
      $(".has-background").css(
        "min-height",
        $(window).height() - $("#page-header").height() + "px"
      );
      $(".has-map.has-sidebar.sidebar-grid .results-wrapper").height(
        $(".hero-section").height() - $(".map-wrapper").height()
      );
    } else {
      if ($("body").hasClass("navigation-fixed")) {
        $(".hero-section.full-screen").height(
          $(window).height() - $("#page-header nav").height()
        );
        $(".hero-section .map-wrapper").css("height", "100%");
      } else {
        $(".hero-section.full-screen").height(
          $(window).height() - $("#page-header").height()
        );
        $(".hero-section .map-wrapper").css("height", "100%");
...
function openModal(target, modalPath, clusterData, mapsFullScreen) {
  if (mapsFullScreen === true) {
    $(".gm-style").append(
      '<div class="modal modal-external fade" id="' +
        target +
    ...
    $.ajax({
      ...
      success: function(results) {
        ...
        if ($("input[type=file]").length) {
          $.getScript("assets/js/jQuery.MultiFile.min.js", function(
            data,
            textStatus,
            jqxhr
          ) {
        ...
        var title = $(results)
          ...
          .text();
        socialShare(
          window.location + "?title=" + title.replace(/\s+/g, "-").toLowerCase()
        );
      ...
      if (_this.find(".map").length) {
        if (_this.find(".modal-dialog").attr("data-address")) {
          simpleMap(
            ...
            0,
            "map-modal",
            _this.find(".modal-dialog").attr("data-marker-drag"),
            _this.find(".modal-dialog").attr("data-address")
          );
        } else {
          simpleMap(
            _this.find(".modal-dialog").attr("data-latitude"),
...
function socialShare(url) {
  ...
  if (socialButtonsEnabled == 1) {
    ...
    $.getScript("assets/js/jssocials.min.js", function(
      data,
      textStatus,
      jqxhr
    ) {
...
function trackpadScroll(method) {
  if (method == "initialize") {
    if ($(".results-wrapper").find("form").length) {
      if (!viewport.is("xs")) {
        $(".results-wrapper .results").height(
          $(".results-wrapper").height() -
  ...
  if (viewport.is("xs")) {
    ...
    if (
      ...
      $(".mega-menu .heading").each(function(e) {
        ...
        $(this)
          .parent()
          .attr("aria-controls", "mega-menu-collapse-" + e);
      });
...
function initializeReadMore() {
  ...
  function readMoreCallBack() {
    ...
    $readMore.readmore({
      ...
      moreLink:
        '<a href="#" class="btn btn-primary btn-xs btn-light-frame btn-framed btn-rounded">More<i class="icon_plus"></i></a>',
      lessLink:
        '<a href="#" class="btn btn-primary btn-xs btn-light-frame btn-framed btn-rounded">Less<i class="icon_minus-06"></i></a>'
    });
  }
}
...
if (!viewport.is("xs")) {
  ...
  $("[data-toggle=popover]").popover({
    template:
      '<div class="popover" role="tooltip"><div class="close"><i class="fa fa-close"></i></div><div class="arrow"></div><h3 class="popover-title"></h3><div class="popover-content"></div></div>'
  });
    ...
    setTimeout(function() {
      if (_this.attr("data-toggle") == "popover") {
        ...
        for (var i = 0; i < messagesArray.length; i++) {
          $(messagesArray[i]).css(
            "bottom",
            parseInt($(messagesArray[i]).css("bottom")) +
              _this.context.clientHeight +
              10
          );
        }
